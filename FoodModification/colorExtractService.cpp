    ...
    for(int y=0; y<hsvImg.rows; y++) {
        for(int x=0; x<hsvImg.cols; x++) {
                if(isNearNormalPointA(x, y, hsvImg, extractParamManager->criterion)) {
                    discriminate(x, y, bgrImg, hsvImg, yCrCbImg, extractParamManager->criterion[0], extractParamManager->colorExtractTolerance[0]);
                } else {
...
bool ColorExtractService::isNearNormalPointA(int x, int y, cv::Mat hsvImg, ColorCriterion* colorCriterion[2]) {

    int absoluteValueDifferenceA = abs(R(hsvImg,x,y) - colorCriterion[0]->getValue());
